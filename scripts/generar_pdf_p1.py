...
_PDF_PARENT = os.path.join(ROOT, "CUADERNILLOS_P1_PDF")
...
PATRON_JSON = os.path.join(DATOS_DIR, "cuadernillo_*_p1_2023_PROPIO.json")
...
def contraportada_flowables(d, st):
    ...
    f.append(HRFlowable(width=CONTENT_W, thickness=0.6, color=C_GRIS,
                        spaceBefore=2, spaceAfter=4, hAlign="CENTER"))
    f.append(Paragraph("Plataforma IDEA · Cuadernillo de banco propio · Período I", st["pie"]))
    return f
...
def main(argv):
    ...
    for ruta in archivos:
        ...
        # cuadernillo_<area>_<g>_p1_2023_PROPIO.json
        m = re.match(r"cuadernillo_([a-z]+)_(\w+)_p1_2023_PROPIO\.json", base)
        if not m:
            continue
